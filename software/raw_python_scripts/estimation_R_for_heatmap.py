    ...
    for ind in data_map.index: 
        ...
        #print(len(comm_obj.infection_rate))
            ...
            if available_in_pop(comm_obj):
                #data_map['Risk-Score'][ind] = (10000)*comm_obj.infection_rate[day]*comm_obj.confirmed_daily[0,day]/(get_population_vec([comm_obj])-comm_obj.confirmed_daily[0,day])
                #data_map['Risk-Level'][ind] = decide_level(data_map['Risk-Score'][ind],thresholds)
                data_map.loc[ind,'Risk-Score'] = (10000)*comm_obj.infection_rate[day]*comm_obj.confirmed_daily[0,day]/(get_population_vec([comm_obj])-comm_obj.confirmed_daily[0,day])
                data_map.loc[ind,'Risk-Level'] = decide_level(data_map['Risk-Score'][ind],thresholds)
...
###==============================================================================
		...
		counter_comm = 0
		print('Number of comm. that their rates have been set:')
		for communiuty_obj in list_selected_communities:
...
# 		return 0  
#		df = pd.read_csv('Covid-19-R-Prediction.csv')
#		df_p = df[df['Risk-Score']==-1]
#		print(df_p.nunique())
		clear_output(wait=True)
